# TODO: make as a rake task

require 'rqrcode_png'
require 'pdf-reader'
require 'zxing'
require_relative 'imagestream'

image_in = ARGV[0]
qrcode_out = ARGV[1]

# require 'qrio'
# puts Qrio::Qr.load("21-3-img2.png").qr.text

# to decode qr
# result = ZXing.decode "1-2-img0.png"
# puts result

# stream = ImageStream.new("21-3-img2.png")
# # if (stream.read_magic)
#   txt = stream.read_string
#
#   qr = RQRCode::QRCode.new(txt, :size => 8)
#   png = qr.to_img
#   png.resize(200, 200).save(qrcode_out)
#
#   puts txt.inspect
# end

# require 'zxing'
# result = ZXing.decode ARGV[0]
# # puts result
#
# # reader = PDF::Reader.new(image_in)
# #
    ...
    def process_page(page, count)
      ...
      xobjects.each do |name, stream|
        ...
        when :Image then
          ...
          when :DCTDecode      then
            ExtractImages::Jpg.new(stream).save("0-#{count}-#{name}.jpg")
          else
  ...
  class Raw
    ...
    def save(filename)
      ...
      when :DeviceRGB  then save_rgb(filename)
        ...
        save_cmyk("1"+filename)
        save_gray("2"+filename)
        save_rgb("3"+filename)
      end
    end
...
PDF::Reader.open(image_in) do |reader|
  page = reader.page(1)
  extractor.page(page)
end
